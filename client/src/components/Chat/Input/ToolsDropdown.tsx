...
const ToolsDropdown = ({ disabled }: ToolsDropdownProps) => {
  ...
  const [chatBadges, setChatBadges] = useRecoilState(store.chatBadges);
  ...
  const isFileSearchPinned = chatBadges.some((b) => b.id === Tools.file_search);
  const isSearchPinned = chatBadges.some((b) => b.id === Tools.web_search);
  const isCodePinned = chatBadges.some((b) => b.id === Tools.execute_code);
  const isArtifactsPinned = chatBadges.some((b) => b.id === AgentCapabilities.artifacts);

  const toggleBadgePin = useCallback(
    (badgeId: string) => {
      setChatBadges((prev) => {
        const isPinned = prev.some((b) => b.id === badgeId);
        if (isPinned) {
          return prev.filter((b) => b.id !== badgeId);
        }
        return [...prev, { id: badgeId }];
      });
    },
    [setChatBadges],
  );
  ...
  const showFileSearch =
    fileSearchEnabled &&
    canUseFileSearch &&
    (!agentToolIds?.size || agentToolIds.has(Tools.file_search));
  if (showFileSearch) {
    dropdownItems.push({
      ...
      render: (props) => (
        <div {...props}>
          ...
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              toggleBadgePin(Tools.file_search);
            }}
            className={cn(
  ...
  const showWebSearch =
    canUseWebSearch &&
    webSearchEnabled &&
    (!agentToolIds?.size || agentToolIds.has(Tools.web_search));
  if (showWebSearch) {
    dropdownItems.push({
        ...
        <div {...props}>
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                toggleBadgePin(Tools.web_search);
              }}
              className={cn(
                'rounded p-1 transition-all duration-200',
                'hover:bg-surface-secondary hover:shadow-sm',
  ...
  const showCode =
    canRunCode &&
    codeEnabled &&
    (!agentToolIds?.size || agentToolIds.has(Tools.execute_code));
  if (showCode) {
    dropdownItems.push({
        ...
        <div {...props}>
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                toggleBadgePin(Tools.execute_code);
              }}
              className={cn(
                'rounded p-1 transition-all duration-200',
                'hover:bg-surface-secondary hover:shadow-sm',
  ...
  const showArtifacts =
    artifactsEnabled &&
    (!agentToolIds?.size || agentToolIds.has(AgentCapabilities.artifacts));
  if (showArtifacts) {
    dropdownItems.push({
      hideOnClick: false,
      render: (props) => (
        <ArtifactsSubMenu
          ...
          setIsArtifactsPinned={() => toggleBadgePin(AgentCapabilities.artifacts)}
