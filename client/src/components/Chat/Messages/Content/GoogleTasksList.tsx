...
/** Compact JSON from API: i=items, i[].id, i[].n=name, i[].s=status, i[].d=date, e=error, p=nextPageToken */
...
function getStatusLabel(status?: string): string {
  ...
  if (status === 'needsAction') return 'To do';
  return status ?? 'To do';
}

const GOOGLE_TASKS_URL = 'https://tasks.google.com/';

export default function GoogleTasksList({
  output,
  initialProgress = 0.1,
  isSubmitting,
  isLast,
  toolCallId,
}: GoogleTasksListProps) {
  ...
  const summary = 'Listed Google tasks';
