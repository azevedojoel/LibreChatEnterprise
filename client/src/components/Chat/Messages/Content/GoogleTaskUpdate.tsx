...
function parseArgs(args: string | Record<string, unknown>): UpdateTaskArgs {
  try {
...
function parseOutput(output: string | null | undefined): UpdateTaskOutput | null {
...
export default function GoogleTaskUpdate({
  args,
  output,
  initialProgress = 0.1,
  isSubmitting,
  ...
  const hasChanges =
    !!parsedArgs.title || !!parsedArgs.notes || !!parsedArgs.status || !!parsedArgs.due;
