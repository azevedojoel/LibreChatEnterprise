...
export default function CalendarGetEvent({
  output,
  initialProgress = 0.1,
  isSubmitting,
  isLast,
  toolCallId,
}: CalendarGetEventProps) {
  ...
  const parsed = useMemo(() => parseCalendarEventOutput(output), [output]);
    ...
    >
      {outputError ? (
        ...
        <div className="space-y-2 text-sm">
          ...
          {event.location && (
            <p className="text-xs text-text-secondary">Location: {event.location}</p>
          )}
        </div>
      ) : null}
    </ToolResultContainer>
