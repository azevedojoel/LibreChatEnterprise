...
import { parseCalendarListEventsOutput } from '~/utils/parseToolOutput';
...
export default function CalendarListEvents({
  ...
  const parsed = useMemo(() => parseCalendarListEventsOutput(output), [output]);
    ...
    >
      {outputError ? (
        ...
        <ul className="space-y-1.5 text-sm">
          {events.map((evt, idx) => {
            const primary = evt.summary ?? 'Untitled';
