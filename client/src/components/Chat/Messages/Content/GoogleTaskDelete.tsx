...
function parseArgs(args: string | Record<string, unknown>): DeleteTaskArgs {
  try {
...
function parseOutput(output: string | null | undefined): DeleteTaskOutput | null {
...
export default function GoogleTaskDelete({
  args,
  output,
  initialProgress = 0.1,
  isSubmitting,
  ...
  const hasExpandableContent = !!parsedArgs.taskId || !!parsedArgs.taskListId || hasOutput;

  const label = parsedArgs.taskId
    ? `task ${parsedArgs.taskId.length > 12 ? `${parsedArgs.taskId.slice(0, 12)}...` : parsedArgs.taskId}`
    : 'task';
  ...
  if (showApprovalBar && isPending) {
    return (
      <ToolApprovalContainer
        ...
        toolName="tasks_deleteTask"
      >
        <div className="space-y-2 text-sm text-text-secondary">
          {parsedArgs.taskId && <div>Task ID: {parsedArgs.taskId}</div>}
