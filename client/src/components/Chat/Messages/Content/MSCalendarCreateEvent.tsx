...
type MSCalendarCreateEventProps = {
...
export default function MSCalendarCreateEvent({
  args,
  output,
  initialProgress = 0.1,
  isSubmitting,
  ...
  const parsedOutput = useMemo(() => parseMSCalendarEventOutput(output), [output]);
  const event = parsedOutput?.event;
  const outputError = parsedOutput?.error;

  const title = event?.subject ?? parsedArgs.subject ?? 'Untitled';
  const summary =
    isLoading || !hasOutput
  ...
  return (
    <ToolResultContainer
      ...
      isLoading={isLoading}
      error={hasError}
      hasExpandableContent={!!event || !!parsedArgs.subject || hasOutput}
      minExpandHeight={120}
    >
      {outputError ? (
