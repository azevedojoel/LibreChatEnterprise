...
function parseArgs(args: string | Record<string, unknown>): CreateTaskArgs {
  try {
...
function parseOutput(output: string | null | undefined): CreateTaskOutput | null {
...
export default function GoogleTaskCreate({
  args,
  output,
  initialProgress = 0.1,
  isSubmitting,
  ...
  const hasExpandableContent = !!parsedArgs.notes || !!parsedArgs.due || hasOutput;
  ...
  if (showApprovalBar && isPending) {
    return (
      <ToolApprovalContainer
        ...
        toolName="tasks_createTask"
      >
        <div className="space-y-2 text-sm">
          <div className="font-medium text-text-primary">{title}</div>
          {parsedArgs.notes && (
            <p className="line-clamp-2 text-text-secondary">{parsedArgs.notes}</p>
          )}
          {parsedArgs.due && (
            <p className="text-xs text-text-secondary">
              Due: {formatDate(parsedArgs.due)}
            </p>
          )}
        </div>
      </ToolApprovalContainer>
