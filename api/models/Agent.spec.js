...
describe('models/Agent', () => {
  ...
  describe('Agent Edge Cases and Error Handling', () => {
    ...
    test('should handle updateAgent with combined MongoDB operators', async () => {
      ...
      // Use multiple operators in single update - direct field update + $push
      const updatedAgent = await updateAgent(
        { id: agentId },
        {
          name: 'Updated Name',
  ...
  describe('Agent IDs Field in Version Detection', () => {
    ...
    test('should skip version creation when skipVersioning option is used', async () => {
      ...
      // Update with skipVersioning - should NOT create a new version
      const updatedWithSkip = await updateAgent(
        { id: agentId },
        { description: 'Updated via skipVersioning' },
        { skipVersioning: true },
      );

      expect(updatedWithSkip.versions).toHaveLength(1);
      expect(updatedWithSkip.description).toBe('Updated via skipVersioning');
