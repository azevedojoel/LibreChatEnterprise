...
import { createScheduledAgentModel } from '../../models/scheduledAgent';
import type { IScheduledAgent } from '../../types/scheduledAgent';

let mongoServer: MongoMemoryServer;
let ScheduledAgent: mongoose.Model<IScheduledAgent>;
...
describe('ScheduledAgent schema - selectedTools', () => {
  const userId = new mongoose.Types.ObjectId();

  it('should store selectedTools as null', async () => {
    const doc = await ScheduledAgent.create({
      userId,
      agentId: 'agent-1',
      name: 'Test',
      prompt: 'Hi',
    ...
    const found = await ScheduledAgent.findById(doc._id).lean();
