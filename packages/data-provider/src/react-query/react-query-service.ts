...
export const useGetWorkflowsQuery = (
  config?: UseQueryOptions<q.TWorkflow[]>,
): QueryObserverResult<q.TWorkflow[]> => {
  return useQuery<q.TWorkflow[]>(
    [QueryKeys.workflows],
    () => dataService.listWorkflows(),
    { refetchOnWindowFocus: false, ...config },
  );
};
...
export const useCreateWorkflowMutation = (): UseMutationResult<
  q.TWorkflow,
  unknown,
  Parameters<typeof dataService.createWorkflow>[0]
> => {
  const queryClient = useQueryClient();
  return useMutation(dataService.createWorkflow, {
    onSuccess: () => {
...
export const useUpdateWorkflowMutation = (): UseMutationResult<
  q.TWorkflow,
  unknown,
  { id: string; data: Parameters<typeof dataService.updateWorkflow>[1] }
> => {
  const queryClient = useQueryClient();
  return useMutation(({ id, data }) => dataService.updateWorkflow(id, data), {
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries([QueryKeys.workflows]);
      queryClient.invalidateQueries([QueryKeys.workflow, variables.id]);
    },
  });
};

export const useDeleteWorkflowMutation = (): UseMutationResult<void, unknown, string> => {
  const queryClient = useQueryClient();
  return useMutation(dataService.deleteWorkflow, {
    onSuccess: () => {
      queryClient.invalidateQueries([QueryKeys.workflows]);
    },
  });
...
export const useGetWorkflowRunsQuery = (
  workflowId: string,
  limit?: number,
  config?: UseQueryOptions<q.TWorkflowRun[]>,
): QueryObserverResult<q.TWorkflowRun[]> => {
  return useQuery<q.TWorkflowRun[]>(
    DynamicQueryKeys.workflowRuns(workflowId, limit),
    () => dataService.getWorkflowRuns(workflowId, limit),
    { enabled: !!workflowId, refetchOnWindowFocus: false, ...config },
  );
};
...
> => {
  const queryClient = useQueryClient();
  return useMutation(
    (workflowId: string) => dataService.runWorkflow(workflowId),
    {
      onSuccess: (_, workflowId) => {
        queryClient.invalidateQueries([QueryKeys.workflow, workflowId]);
...
/* Workflow schedules */
export const useGetWorkflowSchedulesQuery = (
  workflowId: string,
  config?: UseQueryOptions<q.TWorkflowSchedule[]>,
): QueryObserverResult<q.TWorkflowSchedule[]> => {
  return useQuery<q.TWorkflowSchedule[]>(
    DynamicQueryKeys.workflowSchedules(workflowId),
    () => dataService.getWorkflowSchedules(workflowId),
    { enabled: !!workflowId, refetchOnWindowFocus: false, ...config },
  );
};

export const useCreateWorkflowScheduleMutation = (): UseMutationResult<
  q.TWorkflowSchedule,
  unknown,
  { workflowId: string; data: Parameters<typeof dataService.createWorkflowSchedule>[1] }
> => {
  const queryClient = useQueryClient();
  return useMutation(
    ({ workflowId, data }) => dataService.createWorkflowSchedule(workflowId, data),
    {
      onSuccess: (_, variables) => {
        queryClient.invalidateQueries(DynamicQueryKeys.workflowSchedules(variables.workflowId));
        queryClient.invalidateQueries([QueryKeys.workflow, variables.workflowId]);
      },
    },
  );
};

export const useUpdateWorkflowScheduleMutation = (): UseMutationResult<
  q.TWorkflowSchedule,
  unknown,
  { workflowId: string; scheduleId: string; data: Parameters<typeof dataService.updateWorkflowSchedule>[2] }
> => {
  const queryClient = useQueryClient();
  return useMutation(
    ({ workflowId, scheduleId, data }) =>
      dataService.updateWorkflowSchedule(workflowId, scheduleId, data),
    {
      onSuccess: (_, variables) => {
        queryClient.invalidateQueries(DynamicQueryKeys.workflowSchedules(variables.workflowId));
        queryClient.invalidateQueries([QueryKeys.workflow, variables.workflowId]);
...
export const useDeleteWorkflowScheduleMutation = (): UseMutationResult<
  void,
  unknown,
...
> => {
  ...
  return useMutation(
    ({ workflowId, scheduleId }) =>
      dataService.deleteWorkflowSchedule(workflowId, scheduleId),
    {
      onSuccess: (_, variables) => {
        queryClient.invalidateQueries(DynamicQueryKeys.workflowSchedules(variables.workflowId));
        queryClient.invalidateQueries([QueryKeys.workflow, variables.workflowId]);
...
export const useRunWorkflowScheduleMutation = (): UseMutationResult<
  { success: boolean; runId?: string; status?: string; conversationId?: string; error?: string },
  unknown,
  { workflowId: string; scheduleId: string }
> => {
  const queryClient = useQueryClient();
  return useMutation(
    ({ workflowId, scheduleId }) =>
      dataService.runWorkflowSchedule(workflowId, scheduleId),
    {
      onSuccess: (_, variables) => {
        queryClient.invalidateQueries(DynamicQueryKeys.workflowSchedules(variables.workflowId));
        queryClient.invalidateQueries(DynamicQueryKeys.workflowRuns(variables.workflowId));
